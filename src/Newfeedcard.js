...
function Newfeedcard(p) {
  ...
  useEffect(() => {
    let f = async () => {
      ...
      for (let i = 0; i < allCommentId.length; i++) {
        ...
        let doc = await firestore.collection("newfeedcomments").doc(id).get();
        console.log(doc);
  ...
  return (
    <div className="video-container">
      ...
      />
           <b className="Linkusername" onClick={()=>{ 
          console.log(p.post)
          location.push({
        ...
        {boxOpen ? (
          <div className="cmtBox">
            ...
            <button
              ...
              onClick={() => {
                let PromiseOfcmomment = firestore.collection("newfeedcomments").add({
